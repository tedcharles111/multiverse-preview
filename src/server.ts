...
// Manual CORS – runs before everything, sets headers on ALL responses
...
// Logs endpoint (SSE) – now part of previewRoutes, but ensure it's mounted
...
app.use('/preview/:sessionId', async (req, res, next) => {
  ...
  if (!managed || !managed.process || managed.process.exitCode !== null) {
